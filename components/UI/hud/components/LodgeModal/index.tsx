  ...
  const handleConfirmPlace = useCallback(async () => {
    ...
    try {
      ...
      notificationId = showTransactionNotification(notificationDispatch, {
        message: 'Place lodge furniture on GV-2D diamond (Base Sepolia)',
        options: { sound: true },
      });
      const placed = await placeSoftInstallOnDiamond({
        ...
        signer,
        provider,
        name: `Lodge furniture ${pb}`,
      });
      const result = placeLodgeFurniture(current, pb, pending.tx, pending.ty, { skipInventory: true });
    ...
    async (piece: LodgeFurniturePiece) => {
      ...
      if (useDiamond) {
        ...
        try {
          ...
          const parcelKey = interiorParcelKeyLocal('lodge', installationId);
          notificationId = showTransactionNotification(notificationDispatch, {
            message: 'Unequip lodge furniture on GV-2D diamond (move)',
            options: { sound: true },
          });
          ...
          const r = removeLodgeFurniture(current, piece.id, { skipInventory: true });
          if (!r.ok) {
          ...
          // Legacy local-only placement: fall back to local move.
    ...
    async (piece: LodgeFurniturePiece) => {
      ...
      if (useDiamond) {
        ...
        try {
          ...
          const parcelKey = interiorParcelKeyLocal('lodge', installationId);
          notificationId = showTransactionNotification(notificationDispatch, {
            message: 'Unequip lodge furniture on GV-2D diamond',
            options: { sound: true },
          });
          await unequipSoftInstallOnDiamond({
            parcelKey,
