/**
 * MAP_ID_LODGE — the entire map is a 16×16 lodge interior (1024×1024).
 * Client-only: import dynamically from lodge.scene.helper (never from SSR pages).
 */
...
import {
  ...
  lodgeStructureAt,
  lodgeTileCenter,
  type LodgeFurniturePiece,
  type LodgeLayout,
} from 'helpers/lodge.layout.helper';
import { getLocalConsoleUpgradeInfo } from 'helpers/console.installation.helper';
import { sendLodgeMove, getLodgeRoom } from 'helpers/colyseus.lodge';
import type { LodgeSceneBuildState, LodgeSceneCallbacks } from 'helpers/lodge.scene.helper';
import { MAP_CONFIG_BY_ID, MAP_ID_LODGE, TILE_SIZE } from 'shared_code/constants/const.game';
...
export class LodgeScene extends Phaser.Scene {
  private layout: LodgeLayout | null = null;
  private callbacks: LodgeSceneCallbacks | null = null;
  private build: LodgeSceneBuildState = { buildMode: false, placeBrush: null, floorBrush: null, pendingPlace: null };
  ...
  init(data: {
    layout?: LodgeLayout | null;
    callbacks?: LodgeSceneCallbacks;
    build?: LodgeSceneBuildState;
  }) {
  ...
  preload() {
    ...
    if (!this.textures.exists('broadcaster')) {
      this.load.spritesheet('broadcaster', '/animations/installations/broadcaster.png', {
        frameWidth: 256,
        frameHeight: 256,
      });
    }
  ...
  create() {
    const cfg = MAP_CONFIG_BY_ID[MAP_ID_LODGE];
    ...
    const spawn = lodgeTileCenter(LODGE_SPAWN_TX, LODGE_SPAWN_TY);
    ...
    if (this.input.keyboard) {
      ...
      this.wasd = this.input.keyboard.addKeys('W,A,S,D') as LodgeScene['wasd'];
    ...
    this.input.on('pointerdown', (pointer: Phaser.Input.Pointer) => {
      ...
      if (piece && isBroadcasterItemId(piece.itemId)) this.callbacks?.onInteractBroadcaster(piece);
    });
  ...
  update(_t: number, dt: number) {
    ...
    if (vx !== 0 || vy !== 0) {
      ...
      if (now - this.lastMoveSent > 40) {
        ...
        sendLodgeMove(Math.round(resolved.x), Math.round(resolved.y));
      }
    }
    ...
    if (this.time.now > this.doorCooldownUntil) {
      ...
      if (lodgeStructureAt(tx, ty) === 'door' && vy > 0) {
  ...
  setCallbacks(callbacks: LodgeSceneCallbacks) {
  ...
  private textureKeyForItemId(itemId: number): string {
    ...
    if (isBroadcasterItemId(itemId)) return 'broadcaster';
    return 'shelf';
  }
  ...
  private fallbackColorForItemId(itemId: number): number {
    ...
    if (isBroadcasterItemId(itemId)) return 0x4fc3f7;
    return 0x4d96ff;
  }

  private canPlaceAt(tx: number, ty: number): boolean {
    if (tx < 0 || ty < 0 || tx >= LODGE_GRID || ty >= LODGE_GRID) return false;
    if (lodgeStructureAt(tx, ty) !== 'floor') return false;
  ...
  private updatePlacePreview(pointer: Phaser.Input.Pointer) {
    ...
    if (tx < 0 || ty < 0 || tx >= LODGE_GRID || ty >= LODGE_GRID) {
  ...
  private rebuildWorld() {
    ...
    for (let ty = 0; ty < LODGE_GRID; ty += 1) {
      for (let tx = 0; tx < LODGE_GRID; tx += 1) {
        const kind = lodgeStructureAt(tx, ty);
        const cx = tx * TILE_SIZE + TILE_SIZE / 2;
        const cy = ty * TILE_SIZE + TILE_SIZE / 2;
        if (kind === 'wall') {
    ...
    (this.layout?.furniture || []).forEach((piece) => {
      ...
      if (isBroadcasterItemId(piece.itemId)) key = 'broadcaster';
  ...
  private canUpgradePiece(piece: LodgeFurniturePiece): boolean {
  ...
  private setSelectedFurniture(piece: LodgeFurniturePiece) {
    this.clearSelectedFurniture();
  ...
  private resolveMove(fromX: number, fromY: number, toX: number, toY: number): { x: number; y: number } {
    ...
    const tryPos = (x: number, y: number) => {
      ...
      return points.every((p) => {
        ...
        if (!lodgeIsWalkable(tx, ty)) return false;
        // Furniture blocks center but allow door/floor
        if (this.blocked.has(`${tx},${ty}`) && lodgeStructureAt(tx, ty) === 'floor') return false;
        return true;
      });
    };
  ...
  private tryInteract() {
    ...
    else if (isBroadcasterItemId(piece.itemId)) this.callbacks?.onInteractBroadcaster(piece);
  }

  private syncRemotes() {
    const room = getLodgeRoom();
