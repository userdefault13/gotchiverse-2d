...
import {
  ...
  getBazaarJoinError,
  type JoinBazaarOpts,
} from 'helpers/colyseus.bazaar';
import { serializeBazaarLayout, type BazaarLayout } from 'helpers/bazaar.layout.helper';
import { MAP_ID_BAZAAR, MAP_ID_CITAADEL } from 'shared_code/constants/const.game';

export type BazaarSceneCallbacks = {
  onLeaveDoor: () => void;
};

type BazaarSceneApi = {
  setCallbacks: (callbacks: BazaarSceneCallbacks) => void;
  setLayout: (layout: BazaarLayout | null) => void;
};
...
let callbacksRef: BazaarSceneCallbacks | null = null;
...
function getBazaarScene(): BazaarSceneApi | null {
  const game = getGame();
  if (!game) return null;
  try {
    const s = game.scene.getScene(MAP_ID_BAZAAR) as BazaarSceneApi | undefined;
    return s || null;
  } catch {
    return null;
  }
...
export function isBazaarMapActive(): boolean {
  return active || GameController.MAP === MAP_ID_BAZAAR;
}

export function setBazaarSceneCallbacks(callbacks: BazaarSceneCallbacks) {
  callbacksRef = callbacks;
  getBazaarScene()?.setCallbacks(callbacks);
}

export function applyBazaarSceneLayout(layout: BazaarLayout | null) {
  getBazaarScene()?.setLayout(layout);
}

export type EnterBazaarMapOpts = JoinBazaarOpts & {
  layout: BazaarLayout;
  callbacks: BazaarSceneCallbacks;
};

export async function enterBazaarMap(opts: EnterBazaarMapOpts): Promise<{ ok: boolean; error?: string }> {
  ...
  GameController.updateMapType(MAP_ID_BAZAAR);

  const room = await joinBazaarRoom({
    bazaarId: opts.bazaarId,
    ownerAddress: opts.ownerAddress,
    cartridgeId: opts.cartridgeId,
  });
  if (!room) {
    await abortEnter();
    const detail = getBazaarJoinError();
    return {
      ok: false,
      error: detail
        ? `Could not join bazaar: ${detail}`
        : 'Could not join bazaar (full or auth failed).',
    };
  }

  seedBazaarLayout(serializeBazaarLayout(opts.layout));
  ...
  const { BazaarScene } = await import('components/phaser/scenes/bazaarScene');

  if (!game.scene.getScene(MAP_ID_BAZAAR)) {
    game.scene.add(MAP_ID_BAZAAR, BazaarScene, false);
  }

  try {
    game.scene.run(MAP_ID_BAZAAR, data);
  } catch (e) {
    console.warn('enterBazaarMap: scene.run failed, retry add', e);
    game.scene.add(MAP_ID_BAZAAR, BazaarScene, true, data);
  }
...
export async function leaveBazaarMap(): Promise<void> {
  if (!active && GameController.MAP !== MAP_ID_BAZAAR) {
